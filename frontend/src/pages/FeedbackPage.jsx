...
function FeedbackPage() {
  ...
  function handleSend() {
    ...
    setSending(true);
    axios.post(API + '/api/feedback', form, { headers: headers })
      .then(function () { setShowForm(false); setForm({ recipientId: '', type: 'praise', message: '', anonymous: false, visibility: 'private', rating: null }); loadData(); })
  ...
  function handleDelete(id) {
    ...
    axios.delete(API + '/api/feedback/' + id, { headers: headers }).then(function () { loadData(); });
  }
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        ...
        <button className="btn btn--primary" onClick={function () { setShowForm(!showForm); }}>
          ...
          <div className="form-grid">
            <div className="form-group">
              ...
              <select className="form-select" value={form.recipientId} onChange={function (e) { setForm(Object.assign({}, form, { recipientId: e.target.value })); }}>
                ...
                {users.filter(function (u) { return u._id !== user.id; }).map(function (u) { return <option key={u._id} value={u._id}>{u.name} ({u.role})</option>; })}
              </select>
            </div>
            <div className="form-group">
              <label>Type</label>
      ...
      {loading ? (
        <div className="loading-state"><div className="spinner"></div><p>Loading feedback...</p></div>
      ) : feedbacks.length === 0 ? (
        <div className="empty-state"><div className="empty-state__icon">💬</div><h3>No feedback {tab === 'received' ? 'received' : tab === 'sent' ? 'sent' : ''} yet</h3><p>Feedback will appear here once sent or received.</p></div>
      ) : (
            ...
            return (
              <div key={fb._id} className="feedback-item">
                ...
                <div className="feedback-item__actions">
                  ...
                  {(tab === 'sent' || user.role === 'ADMIN') && <button className="btn btn--ghost btn--sm" onClick={function () { handleDelete(fb._id); }}>Delete</button>}
                </div>
              </div>
            );
          })}
